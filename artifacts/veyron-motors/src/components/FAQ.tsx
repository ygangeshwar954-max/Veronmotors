    ...
    <section id="faq" className="py-[110px] bg-[#111111]">
      <div className="container mx-auto px-6 md:px-20 max-w-[1280px]">
        <div className="grid grid-cols-1 md:grid-cols-[1fr_1.6fr] gap-12 md:gap-[88px] items-start">
          ...
          {/* Sticky sidebar */}
          <Reveal direction="left" delay={0} duration={0.85}>
            <div className="md:sticky md:top-[100px]">
              ...
              <button className="btn-outline">Contact an Advisor</button>
            </div>
          </Reveal>

          {/* Accordion */}
          <Reveal direction="right" delay={0.14} duration={0.85}>
            <div>
              {faqs.map((faq, i) => {
                ...
                return (
                  <div key={i} className="border-b border-white/5 overflow-hidden">
                    ...
                    >
                      ...
                      <div className={`w-[26px] h-[26px] rounded-full border flex items-center justify-center shrink-0 text-[18px] leading-none transition-all duration-300 ${isOpen ? 'border-[#C9A84C] text-[#C9A84C] rotate-45' : 'border-white/15 text-white/50'}`}>
                        +
                      </div>
                    </button>
                    <AnimatePresence>
                      {isOpen && (
                        ...
                        >
                          <div className="pb-6 text-[14px] leading-[1.85] text-white/50">{faq.a}</div>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  </div>
